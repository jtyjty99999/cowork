...
**功能描述**：
实现了类似 Claude Code 的 Skills 系统，允许用户通过 SKILL.md 文件定义可复用的任务模板和工作流程。

**核心特性**：
- ✅ **Skill 定义**：使用 Markdown + YAML frontmatter 格式定义 Skills
- ✅ **斜杠命令调用**：通过 `/skill-name arguments` 格式调用 Skills
- ✅ **自动发现**：自动扫描 `.cowork/skills/` 目录加载 Skills
- ✅ **参数替换**：支持 `$ARGUMENTS`、`$ARGUMENTS[N]`、`$N` 格式的参数
- ✅ **工具限制**：可通过 `allowed-tools` 限制 Skill 可使用的工具
- ✅ **调用控制**：支持 `user-invocable` 和 `disable-model-invocation` 控制调用权限
- ✅ **AI 自动匹配**：AI 可根据用户意图自动使用合适的 Skill
...
**内置示例 Skills**：
- `/explain-code` - 代码解释（使用类比和图表）
- `/code-review` - 代码审查（质量、安全、性能）
- `/deploy` - 部署应用（仅用户可调用）
- `/fix-issue` - 修复 GitHub Issue
- `api-conventions` - API 设计规范（背景知识）

**UI 展示**：
- Skill 调用卡片显示执行状态、描述和允许的工具
- 快速任务入口包含 Skill 测试选项
- 进度条显示 Skill 调用过程

**新增文件**：
- `lib/skills/` - Skills 核心模块（types, loader, registry, executor, discovery）
- `app/api/skills/` - Skills API 路由（discover, files, read-file）
- `hooks/useSkills.ts` - Skills 管理 Hook
- `components/SkillCallDisplay.tsx` - Skill 调用展示组件
- `components/SkillsList.tsx` - Skills 列表组件
- `.cowork/skills/` - 示例 Skills
- `docs/SKILLS_GUIDE.md` - Skills 使用指南

**截图**：

![Skill 调用示例](./screenshots/skill-call-example.png)
